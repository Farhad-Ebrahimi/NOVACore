...
void fpu_test_bench() {
    ...
    // These write 0 or 1 to integer registers [cite: 66, 125]
    int eq = (a.f == b.f);
    int lt = (a.f < b.f);
    int le = (a.f <= b.f);

    // 4. Conversions (FCVT.S.W, FCVT.W.S) [cite: 66, 122, 123]
    int int_val = 10;
    float f_from_int = (float)int_val; // fcvt.s.w
    int int_from_f = (int)f_mul;       // fcvt.w.s

    // 5. Integer Store (Final bit pattern output via sw)
