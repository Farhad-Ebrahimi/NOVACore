...
int int_alu_test(float a, float b)
{

    volatile int int_a = (int)a; // fcvt.w.s
    volatile int int_b = (int)b; // fcvt.w.s

    volatile int result_add = int_a + int_b; // add
...
//
//volatile int sum = num1 + num2; 


volatile float a = 2.50;
volatile float b = 3.75;
volatile float sum = a + b;


//volatile float fnum1 = (float)num1; // fcvt.s.w
//volatile float fnum2 = (float)num2; // fcvt.s.w
//
//volatile int sum = int_alu_test(fnum1, fnum2); // This will convert floats back to ints and add them 
