...
void fpu_test_bench() {
   ...
   volatile  // 3. Comparisons (FEQ, FLT, FLE)
   volatile  // These write 0 or 1 to integer registers [cite: 66, 125]
   volatile  int eq = (a.f == b.f);
   volatile  int lt = (a.f < b.f);
   volatile  int le = (a.f <= b.f);
 
   volatile  // 4. Conversions (FCVT.S.W, FCVT.W.S) [cite: 66, 122, 123]
   volatile  int int_val = 10;
   volatile  float f_from_int = (float)int_val; // fcvt.s.w
   volatile  int int_from_f = (int)f_mul;       // fcvt.w.s

   volatile  // 5. Integer Store (Final bit pattern output via sw)
   volatile  res.f = f_mul + f_from_int;
