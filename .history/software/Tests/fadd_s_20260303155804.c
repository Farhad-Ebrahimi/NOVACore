...
int main() {
   ...
   volatile int b = 20;
    ...
    // --- This forces fcvt.s.w ---
  volatile  float fa = (float)a;
    volatile float fb = (float)b;

    // --- Floating-point addition ---
    volatile float sum = fa + fb;   // should be 30.0

    // --- This forces fmv.x.w ---
    // reinterpret float bits as int
   volatile int raw_bits = *(int*)&sum;

    // --- This forces fmv.w.x ---
    // reinterpret int bits back to float
    volatile float recovered = *(float*)&raw_bits;

    // --- Convert back to int ---
    volatile int result = (int)recovered;  // should be 30
