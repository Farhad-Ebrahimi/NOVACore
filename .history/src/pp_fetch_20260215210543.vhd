...
begin
	...
	instruction_data <= imem_data_in when ( stall = '0' AND stall_fpu ='0' ) and imem_ack='1'  else imem_data;
	instruction_ready <= imem_ack and ((not stall) and (not stall_fpu)) and (not cancel_fetch) ;
	...
	begin
		...
		elsif imem_ack = '1' and (stall = '0' and stall_fpu = '0') and cancel_fetch = '0' then
			pc_next <= predicted_target;
		else
			pc_next <= pc;
		end if;
