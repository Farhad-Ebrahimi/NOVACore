...
-------------------------------------------------------------------
  ...
  -- ILA debug: forwarded FP operands
  attribute MARK_DEBUG of frs1_forwarded       : signal is "TRUE";
  attribute MARK_DEBUG of frs2_forwarded       : signal is "TRUE";
  -- ILA debug: FPU result at stg3
  attribute MARK_DEBUG of frd_data_stg3        : signal is "TRUE";
  -- ILA debug: integer ALU result at stg2 (carries FCVT_S_W)
  attribute MARK_DEBUG of bw_alu_result_to_stg3 : signal is "TRUE";
  -- ILA debug: alu operation tracking
  attribute MARK_DEBUG of alu_op_to_stg2       : signal is "TRUE";
  attribute MARK_DEBUG of alu_op_to_stg3       : signal is "TRUE";
...
if frd_write_to_stg3 = '1' and frd_addr_to_stg3 = frs2_addr then ----and (not is_fp_op(alu_op_to_stg3)) then
         frs2_forwarded <= bw_alu_result_to_stg3;

elsif frd_write_to_forwarding_stg3 = '1' and frd_addr_to_forwarding_stg3 = frs2_addr then ----and (not is_fp_op(alu_op_to_forwarding_stg3)) then
...
when (
    (
        frd_write_to_stg2 = '1'
        and (not(is_fp_op(alu_op_to_stg2)))
        and (
            frd_addr_to_stg2 = frs1_addr_in
         or frd_addr_to_stg2 = frs2_addr_in
        )
    )
    or
    (
        frd_write_to_stg3 = '1'
        and (not(is_fp_op(alu_op_to_stg3)))
        and (
            frd_addr_to_stg3 = frs1_addr_in
         or frd_addr_to_stg3 = frs2_addr_in
        )
    )
)
else '0';
