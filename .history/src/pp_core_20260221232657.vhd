...
---------------------------------need to check the stall for csr read address------------------------
	...
	frs1_address <= id_frs1_address when () else
		frs1_address_p;
	frs2_address <= id_frs2_address when ((not internal_stall) and (not div_stall)) else
		frs2_address_p;



	store_previous_frsaddr : process (clk, internal_stall, div_stall)
		begin
		if rising_edge(clk) and (internal_stall = '0') and (div_stall = '0') then
