...
begin
  ...
  -- rd_data_out routing: select between ALU result, FMV data, and FCVT results
  rd_data_out <= frs1_forwarded when alu_op_in = ALU_FMVXW else
                 fcvt_int_result when alu_op_in = ALU_FCVT_W or alu_op_in = ALU_FCVT_WU else
                 alu_result;
  
  -- frd_data_out routing: FMV.W.X writes integer register to FP register file
  frd_data_out <= rs1_forwarded when alu_op_in = ALU_FMVWX else
                  fcvt_fp_result when alu_op_in = ALU_FCVT_S_W or alu_op_in = ALU_FCVT_S_WU else
                  (others => '0');
