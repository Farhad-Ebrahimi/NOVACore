...
begin
	...
	begin
		if rising_edge(clk) then
			...
			elsif flush = '1' or instruction_ready = '0' then -----or insert_nop = '1'  then ----or insert_nop = '1'  then    ---- or insert_nop = '1'
				instruction <= RISCV_NOP;
				count_instruction <= '0';
			else
...
--	-- Extract register addresses from the instruction word:
	...
	rs1_addr <= instruction(19 downto 15) when instruction(6 downto 2) /= b"10100" else 
	            instruction(19 downto 15) when (instruction(6 downto 2) = b"10100" and instruction(31 downto 25) = b"1111000") else  -- FMV.W.X: read from int rs1
	            (others => '0');  --source register 1
	rs2_addr <= instruction(24 downto 20) when instruction(6 downto 2) /= b"10100" else (others => '0');  --source register 2
	rd_addr  <= instruction(11 downto  7) when instruction(6 downto 2) /= b"10100" else 
	            instruction(11 downto 7) when (instruction(6 downto 2) = b"10100" and instruction(31 downto 25) = b"1110000") else  -- FMV.X.W: write to int rd
	            (others => '0');   --destination register
	rd_addr_reg <= instruction(11 downto  7) when instruction(6 downto 2) /= b"10100" else 
	               instruction(11 downto 7) when (instruction(6 downto 2) = b"10100" and instruction(31 downto 25) = b"1110000") else  
	               (others => '0');  
	---------For floating pont operations------------
		...
		-- Extract register addresses from the instruction word:
	frs1_addr <= instruction(19 downto 15) when (instruction(6 downto 2) = b"10100" and instruction(31 downto 25) /= b"1111000") else  -- All FP ops except FMV.W.X
	             (others => '0');  --source register 1
	frs2_addr <= instruction(24 downto 20) when instruction(6 downto 2) = b"10100" else (others => '0');  --source register 2
	frd_addr  <= instruction(11 downto  7) when (instruction(6 downto 2) = b"10100" and instruction(31 downto 25) /= b"1110000") else  -- All FP ops except FMV.X.W
	             (others => '0');   --destination register
	frd_addr_reg <= instruction(11 downto  7) when (instruction(6 downto 2) = b"10100" and instruction(31 downto 25) /= b"1110000") else  
	                (others => '0');
...
begin
	...
	--div_detected_out <= '1' when (alu_op_reg = ALU_FDIV) and (frd_write_reg = '1') and (stall_fpu = '0')  else '0';  ---and (decode_valid_out = '1')
