...
entity fp_exe_stg2 is
  port (
    ...
    en_fadd_out : out std_logic;
    en_fsub_out : out std_logic;
    en_fmul_out : out std_logic;
    en_fdiv_out : out std_logic;
...
architecture behaviour of fp_exe_stg2 is
  ...
  ---signal div_s1_valid_in : std_logic;  ----from stage1 to stage2;
    ...
    -----------internal signals for fmul.s-----




 signal    	mul_A_sign     :    std  logic;
 signal mul_B_sign     :    std  logic;
 signal mul_A_exp     :     std  logic  vector(7 downto 0);
 signal mul_B_exp     :     std  logic  vector(7 downto 0);
 signal mul_A_frac     :    std  logic  vector(22 downto 0);  -- Raw fraction for NaN/  f detection
 signal mul_B_frac     :    std  logic  vector(22 downto 0);
 signal mul_A_mant     :    std  logic  vector(26 downto 0);
 signal mul_B_mant     :    std  logic  vector(26 downto 0);
 signal mul_rm     :        std  logic  vector(2 downto 0);
...
begin
  ...
  en_fadd_out <= en_fadd_reg;
  en_fsub_out <= en_fsub_reg;
  en_fmul_out <= en_fmul_reg;
  en_fdiv_out <= en_fdiv_reg;
