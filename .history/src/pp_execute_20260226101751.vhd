...
--------------------------------------------------------------------------------
    ...
    -- Enable signals for power gating (monitoring)
...
frd_data_out <= frd_data_stg3; 
  ...
  begin
    if rising_edge(clk) then
      ...
      if stall_exe_stg1 = '0' and internal_stall = '0' then
...
--    end if;                        
 ...
 begin
  ...
  begin
    ...
    if (mem_op_to_stg3 = MEMOP_TYPE_LOAD or mem_op_to_stg3 = MEMOP_TYPE_LOAD_FP or mem_op_to_stg3 = MEMOP_TYPE_LOAD_UNSIGNED) and
      ((alu_x_src = ALU_SRC_REG and rd_addr_to_stg3 = rs1_addr and rs1_addr /= b"00000" ) or (frd_addr_to_stg3 = frs1_addr)  or 
      (alu_y_src = ALU_SRC_REG and rd_addr_to_stg3 = rs2_addr and rs2_addr /= b"00000") or ( frd_addr_to_stg3 = frs2_addr )) then

      load_hazard_detected <= '1';

    elsif (mem_op_to_hazard_stg3 = MEMOP_TYPE_LOAD or mem_op_to_hazard_stg3 = MEMOP_TYPE_LOAD_FP or mem_op_to_hazard_stg3 = MEMOP_TYPE_LOAD_UNSIGNED) and
      ((alu_x_src = ALU_SRC_REG and rd_addr_to_forwarding_stg3 = rs1_addr and rs1_addr /= b"00000") or (frd_addr_to_forwarding_stg3 = frs1_addr ) or 
      (alu_y_src = ALU_SRC_REG and rd_addr_to_forwarding_stg3 = rs2_addr and rs2_addr /= b"00000") or (frd_addr_to_forwarding_stg3 = frs2_addr )) then
