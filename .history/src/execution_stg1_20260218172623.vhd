...
-------- need to check for floats not yet for floats
  ...
  begin
    ...
    int2float : entity work.pp_int_2_float is
    port(
        data_in => open,    --! Input integer in two's complement (or unsigned) format.
        alu_op_in => alu_op,                   --! ALU operation (ALU_FCVT_S_W or ALU_FCVT_S_WU).
        frd_data_out => open  --! Output floating-point number in IEEE 754 format.
    );
