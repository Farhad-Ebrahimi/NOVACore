...
begin
	...
	stall_fetch <= stall or stall_fpu;  ---stall or stall_fpu
	
	
	instruction_data <= imem_data_in when ( stall_fetch = '0') and imem_ack='1'  else imem_data;  ---or stall_fpu ='0'
	instruction_ready <= imem_ack and ((not stall_fetch)) and (not cancel_fetch) ;  ----or (not stall_fpu)
	...
	begin
		if rising_edge(clk) then
			if reset = '1' then
				...
				if ( stall_fetch = '0') and imem_ack = '1' then   -----or stall_fpu ='0'
	...
	calc_next_pc : process (reset, stall_fetch, exception, imem_ack, evec, pc, cancel_fetch, wrong_prediction, predicted_target)
	begin
		if exception = '1' then
			pc_next <= evec;
		elsif wrong_prediction = '1' then
			pc_next <= predicted_target;
		elsif imem_ack = '1' and (stall_fetch = '0') and cancel_fetch = '0' then   ----or stall_fpu = '0'
			pc_next <= predicted_target;
		else
			pc_next <= pc;
		end if;
