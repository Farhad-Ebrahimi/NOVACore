...
---------jump not for floats
  ...
  dmem_address <= fp_mem_addr when (mem_op_in = MEMOP_TYPE_LOAD_FP or mem_op_in = MEMOP_TYPE_STORE_FP) else (others => '0');
  --dmem_address <= (others => '0');
  --dmem_data_out  <= frs2_forwarded when mem_op = MEMOP_TYPE_STORE_FP else rs2_forwarded;
  dmem_data_out <= frs2_forwarded when (mem_op_in = MEMOP_TYPE_LOAD_FP or mem_op_in = MEMOP_TYPE_STORE_FP) else rs2_forwarded; -- select FP store data when opcode matches
  --dmem_data_out <= rs2_forwarded;   ---- fpu
  dmem_write_req <= '1' when (mem_op_in = MEMOP_TYPE_STORE or mem_op_in = MEMOP_TYPE_STORE_FP) and exception_taken = '0' else '0';
  dmem_read_req <= '1' when memop_is_load(mem_op_in) and exception_taken = '0' else '0';
...
fp_immediate <= (31 downto 12 => instruction_in(31)) & instruction_in(31 downto 20)
                  when (instruction_in(6 downto 2) = b"00001") else  -- FLW: I-type
                (31 downto 12 => instruction_in(31)) & instruction_in(31 downto 25) & instruction_in(11 downto 7)
                  when (instruction_in(6 downto 2) = b"01001") else  -- FSW: S-type
...
fp_mem_addr <= std_logic_vector(unsigned(rs1_forwarded) + unsigned(fp_immediate));
  ...
  find_exception_cause : process (decode_exception, decode_exception_cause, mem_op_in,
  ...
  begin
    ...
    elsif mem_op_in = MEMOP_TYPE_INVALID then
...
--- fp_alu_x_mux : entity work.pp_alu_mux
---   port map(
---     source => alu_x_src,
---     register_value => frs1_forwarded,
---     immediate_value => (others => '0'),
---     shamt_value => (others => '0'),
---     pc_value => pc,
---     csr_value => csr_value,
---     output => alu_x_mux_o
---   );

-- fp_alu_y_mux : entity work.pp_alu_mux
--   port map(
--     source => alu_y_src,
--     register_value => frs2_forwarded,
--     immediate_value => (others => '0'),
--     shamt_value => (others => '0'),
--     pc_value => pc,
--     csr_value => csr_value,
...
---- will check later not needed. 
  ...
  -- fp_csr_alu_instance : entity work.pp_csr_alu 
  -- port map(
  --   x => csr_value,
  --   y => frs1_forwarded,
  --   result => csr_value_out,
