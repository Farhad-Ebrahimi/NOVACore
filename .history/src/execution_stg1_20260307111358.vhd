...
begin
  ...
  frd_data_out <= frs1_conv when alu_op = ALU_FCVT_S_W or alu_op = ALU_FCVT_S_WU 
                            else rs1_forwarded when alu_op = ALU_FMVWX
                            else (others => '0');
...
---------jump not for floats
  ...
  dmem_fpdata_out <= frs2_forwarded;
  ...
  begin
    if rising_edge(clk) then
      if reset = '1' or flush = '1' then
        ...
        csr_write <= CSR_WRITE_NONE; 
      elsif stall = '0' and internal_stall = '0' then  
 ...
 ----- pro not needed for fpu
  ...
  fpu_stg1_instance : entity work.fpu_stage1
    port map(
      ...
      en_fadd_in => en_fadd_reg,---ignore these enable signals
      en_fsub_in => en_fsub_reg,---ignore these enable signals
      en_fmul_in => en_fmul_reg,---ignore these enable signals
      en_fdiv_in => en_fdiv_reg,---ignore these enable signals
