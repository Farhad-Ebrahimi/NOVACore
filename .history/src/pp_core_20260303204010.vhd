...
architecture behaviour of pp_core is
	...
	------- Stall signals -------
	signal stall_if, stall_id, stall_ex, stall_mem : STD_LOGIC;   ----normal stall for if,id,ex and mem
	...
	signal fp_raw_hazard_detected : std_logic;  
	...
	signal raw_stall_counter : integer range 0 to 3 := 0;  
	signal fpu_stall_active : std_logic := '0';  
	signal stall_ex_stg1 : std_logic;

	signal immediate_fp : std_logic_vector(31 downto 0);  -- Immediate for floating-point instructions, coming from immediate decoder
	
	--------------------------------------------------------------------
	-- ILA debug attributes
	attribute mark_debug : string;
	-- Pipeline tracking
	attribute mark_debug of decode_instruction_out : signal is "true";
	-- FP register file raw outputs (what DIV currently latches)
	attribute mark_debug of frs1_data              : signal is "true";
	attribute mark_debug of frs2_data              : signal is "true";
	attribute mark_debug of frs1_address           : signal is "true";
	attribute mark_debug of frs2_address           : signal is "true";
	-- FP writeback
	attribute mark_debug of wb_frd_address         : signal is "true";
	attribute mark_debug of wb_frd_data            : signal is "true";
	attribute mark_debug of wb_frd_write           : signal is "true";
	-- Stalls
	attribute mark_debug of stall_ex               : signal is "true";
	attribute mark_debug of stall_mem              : signal is "true";
	attribute mark_debug of internal_stall         : signal is "true";
	attribute mark_debug of stall_fpu              : signal is "true";
	attribute mark_debug of delibrate_stall        : signal is "true";
	-- Data memory bus
	attribute mark_debug of ex_dmem_address        : signal is "true";
	attribute mark_debug of ex_dmem_data_out       : signal is "true";
	attribute mark_debug of dmem_data_in_r         : signal is "true";
	attribute mark_debug of ex_dmem_write_req      : signal is "true";
	attribute mark_debug of ex_dmem_read_req       : signal is "true";
	-- Execute stage FP result
	attribute mark_debug of ex_frd_data            : signal is "true";
	attribute mark_debug of ex_frd_address         : signal is "true";
	attribute mark_debug of ex_frd_write           : signal is "true";
	--------------------------------------------------------------------
    
begin

	stall_if <=  stall_id or delibrate_stall or insert_nop_id;
	stall_id <= stall_ex ; ----or fpu_stall_active;    
	...
	stall_mem <= to_std_logic(memop_is_load(mem_mem_op) and (dmem_read_ack_r = '0'))
		...
		or to_std_logic((mem_mem_op = MEMOP_TYPE_STORE_FP) and (dmem_write_ack_r = '0'));  

	 
	----stall_ex_stg1 <= stall_ex or fpu_stall_active;
...
---------------------------------need to check the stall for csr read address------------------------
	...
	frs1_address <= id_frs1_address when stall_fpu = '0' and stall_ex = '0' else
		frs1_address_p;
	frs2_address <= id_frs2_address when stall_fpu = '0' and stall_ex = '0'  else
		frs2_address_p;



	store_previous_frsaddr : process (clk, stall_fpu,stall_ex)
		begin
		if rising_edge(clk) and stall_fpu = '0' and stall_ex = '0' then  ---and fpu_stall_active = '0'
...
---------------------------------------------------------------------
	...
	execute : entity work.pp_execute
		port map(
			...
			fp_raw_hazard_detected => fp_raw_hazard_detected,
			flush => flush_ex,
			irq => irq,
...
------------------------need to check the stall condition ----------------
	dmem_address <= ex_dmem_address when (stall_mem = '0' and internal_stall = '0') else
		dmem_address_p;
	dmem_data_size <= ex_dmem_data_size when (stall_mem = '0' and internal_stall = '0') else
		dmem_data_size_p;
	dmem_data_out <= ex_dmem_data_out when (stall_mem = '0' and internal_stall = '0') else
		dmem_data_out_p;
	dmem_read_req <= ex_dmem_read_req when (stall_mem = '0' and internal_stall = '0') else
		dmem_read_req_p;
	dmem_write_req <= ex_dmem_write_req when (stall_mem = '0' and internal_stall = '0') else
		dmem_write_req_p;

	store_previous_dmem_address : process (clk, stall_mem)
	begin
		if rising_edge(clk) and stall_mem = '0' and internal_stall = '0' then
	...
	memory : entity work.pp_memory
		port map(
			...
			dmem_read_ack => dmem_read_ack_r,   ----going into memory stage
...
----raw_stall_proc : process(clk)
----begin
----    if rising_edge(clk) then
----        if reset = '1' then
----            raw_stall_counter <= 0;
----        else
----            if raw_stall_counter > 0 then
----                -- Decrement the counter if we are already stalling
----                raw_stall_counter <= raw_stall_counter - 1;
----            elsif fp_raw_hazard_detected = '1' then
----                -- Start a 2-cycle stall
----                -- We set it to 2 so it stays active for the remainder 
----                -- of this cycle and the next full cycle.
----                raw_stall_counter <= 4; 
----            end if;
----        end if;
----    end if;
----end process raw_stall_proc;

-- Create the actual stall signal based on the counter
-- This goes to stall_id and stall_if
---fpu_stall_active <= '1' when ( fp_raw_hazard_detected = '1') else '0'; --raw_stall_counter > 0 or
