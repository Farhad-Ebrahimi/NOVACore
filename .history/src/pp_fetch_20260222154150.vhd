...
begin
	...
	instruction_data <= imem_data_in when ( stall = '0') and imem_ack='1'  else imem_data;  ---or stall_fpu ='0'
	instruction_ready <= imem_ack and ((not stall) ) and (not cancel_fetch) ;  ----or (not stall_fpu)
	...
	begin
		...
		elsif imem_ack = '1' and (stall = '0' ) and cancel_fetch = '0' then   ----or stall_fpu = '0'
			pc_next <= predicted_target;
		else
			pc_next <= pc;
		end if;
