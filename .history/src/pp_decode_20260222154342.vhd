...
architecture behaviour of pp_decode is
	...
	signal decode_funct7 : std_logic_vector(6 downto 0);  --added for fpu
...
begin
	...
	decode_funct7 <= instruction(31 downto 25);
	...
	begin
		if rising_edge(clk) then
			if reset = '1' then
				...
				pc <= RESET_ADDRESS;
				count_instruction <= '0';
			elsif stall = '1'  then   -----or stall_fpu = '1'so when division is detected we hold the pc and stall deocde
...
begin
	...
	decode_valid <= instruction_ready ; --when ( (stall = '0')) and flush = '0' else '0'; ---(stall_fpu = '0') or ---when ((stall_fpu = '0') or (stall = '0')) and flush = '0' else '0';
	instruction_out <= instruction;   ----32 bit riscv intruction


	-- DIV Detection (combinational)  only for fpus
	div_detected_out <= '1' when (decode_funct7 = "0001100") and 
