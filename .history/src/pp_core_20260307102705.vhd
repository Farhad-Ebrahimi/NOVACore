...
begin
	...
	stall_id <= stall_ex or fp_raw_hazard_detected;   -- RAW HAZARD is taken care of by Nops and Forwarding  
...
---------------------------------need to check the stall for csr read address------------------------
	...
	frs1_address <= id_frs1_address when internal_stall = '0' and stall_ex = '0' else
		frs1_address_p;
	frs2_address <= id_frs2_address when internal_stall = '0' and stall_ex = '0'  else
		frs2_address_p;



	store_previous_frsaddr : process (clk, internal_stall,stall_ex)
		begin
		if rising_edge(clk) and internal_stall = '0' and stall_ex = '0' then 
	...
	decode : entity work.pp_decode
		generic map(
			...
			immediate_fp => immediate_fp,  -- Immediate for floating-point instructions ---not used redundant
...
----- we dont need to stall decode and fetch at all but still increased the stall counter just in case.
...
begin
    if rising_edge(clk) then
        ...
        elsif (div_counter >= 5) then
            -- counter >= 5: Waiting for division to complete
            if (div_done = '1') then
