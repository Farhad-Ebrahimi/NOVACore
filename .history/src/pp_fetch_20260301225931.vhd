...
entity pp_fetch is
	...
	port (
		...
		fp_raw_hazard_detected : in std_logic;  ---added 2026
...
begin
	...
	instruction_data <= imem_data_in when ( stall = '0' and stall_fpu ='0' and fp_raw_hazard_detected = '0') and imem_ack='1'  else imem_data;  ---or stall_fpu ='0'
	instruction_ready <= imem_ack and ((not stall) and (not stall_fpu) and (not fp_raw_hazard_detected)) and (not cancel_fetch) ;  ----or (not stall_fpu)
	...
	begin
		if rising_edge(clk) then
			if reset = '1' then
				...
				if ( stall = '0' and stall_fpu = '0' and fp_raw_hazard_detected = '0') and imem_ack = '1' then   -----or stall_fpu ='0'
	...
	begin
		...
		elsif imem_ack = '1' and (stall = '0' and stall_fpu = '0' and fp_raw_hazard_detected = '0') and cancel_fetch = '0' then   ----or stall_fpu = '0'
			pc_next <= predicted_target;
		else
			pc_next <= pc;
		end if;
