...
begin
	...
	instruction_data <= imem_data_in when ( stall = '0')  or (internal_stall='0' and div_stall='0' ) and imem_ack='1'  else imem_data;
	...
	begin
		if rising_edge(clk) then
			if reset = '1' then
				...
				if ( stall = '0')  or (internal_stall='0' and div_stall='0' ) and imem_ack = '1' then  ----or stall_fpu ='0'
	...
	calc_next_pc : process (reset, stall,stall_fpu, internal_stall, div_stall, exception, imem_ack, evec, pc, cancel_fetch, wrong_prediction, predicted_target)
	begin
		if exception = '1' then
			pc_next <= evec;
		elsif wrong_prediction = '1' then
			pc_next <= predicted_target;
		elsif imem_ack = '1' and ( stall = '0')  or (internal_stall='0' and div_stall='0' ) and cancel_fetch = '0' then
			pc_next <= predicted_target;
		else
			pc_next <= pc;
		end if;
