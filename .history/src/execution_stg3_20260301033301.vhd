...
-- FPU Stage 2 all 4 operations inputs (for final muxing by funct7)
    ...
    signal add_exp:  std_logic_vector(7 downto 0);
    signal add_mant :  std_logic_vector(27 downto 0);
    signal add_rm :  std_logic_vector(2 downto 0);
    signal add_valid :  std_logic;
    signal add_s1_valid_in :   std_logic;  ---- coming from stage2
     -- SUB Stage 2 inputs
    signal sub_sign :  std_logic;
    signal sub_exp :  std_logic_vector(7 downto 0);
    signal sub_mant :  std_logic_vector(27 downto 0);
    signal sub_rm :  std_logic_vector(2 downto 0);
     
    --signal -- MUL Stage 2 inputs
    signal mul_product :  std_logic_vector(47 downto 0);
    signal mul_exp_sum :  std_logic_vector(7 downto 0);
    signal mul_sign :  std_logic;
    signal mul_a_is_nan_r :  std_logic;
    signal mul_b_is_nan_r :  std_logic;
    signal mul_a_is_inf_r :  std_logic;
    signal mul_b_is_inf_r :  std_logic;
    signal mul_a_is_zero_r :  std_logic;
    signal mul_b_is_zero_r :  std_logic;
    signal mul_rm :  std_logic_vector(2 downto 0);
    
    --signal -- DIV Stage 2 inputs
    signal div_quotient : std_logic_vector(31 downto 0);
    signal div_sticky : std_logic;
    signal div_exp : std_logic_vector(7 downto 0);
    signal div_sign : std_logic;
    signal div_special : std_logic;
    signal div_special_result : std_logic_vector(31 downto 0);
    signal div_valid : std_logic;
    signal div_rm : std_logic_vector(2 downto 0);
    signal div_s1_valid_in :  std_logic;  ---- coming from stage2
    signal div_busy_in        : std_logic;

    signal start_in        : std_logic;
     
    signal instruction :  std_logic_vector(31 downto 0);  -- For funct7
...
begin
      ...
      if reset = '1' then
       ...
       --- frd_data_out <= frd_data_in;   ---- for FPU
        ...
        dmem_read_req  <= dmem_read_req_in;
              ...
              add_rm <= add_rm_in;
              add_valid <= add_valid_in;
              add_s1_valid_in <= add_s1_valid_out;
              ...
              mul_a_is_nan_r <= mul_a_is_nan;
              mul_b_is_nan_r <= mul_b_is_nan; 
              mul_a_is_inf_r <= mul_a_is_inf;
              mul_b_is_inf_r <= mul_b_is_inf; 
              mul_a_is_zero_r <= mul_a_is_zero;
              mul_b_is_zero_r <= mul_b_is_zero;
              mul_rm <= mul_rm_in;
              -- DIV Stage 2 inputs (capture from entity ports into registered internal signals)
              ...
              div_rm <= div_rm_in;
              div_s1_valid_in <= div_s1_valid_out;
  ...
  fpu_stg3_instance : entity work.fpu_stage3
    port map(
      ...
      funct7 => instruction(31 downto 25),  -- Operation selector for muxing (combinatorial from decode)
      -- ADD Stage 2 inputs
      add_sign => add_sign,
      add_exp => add_exp,
      add_mant => add_mant,
      add_rm => add_rm,
      add_valid => add_valid,  ---from stg2
      add_s1_valid_out => add_s1_valid_in,  ----from stg1
      ...
      mul_sign => mul_sign,
      mul_a_is_nan => mul_a_is_nan_r,
      mul_b_is_nan => mul_b_is_nan_r,
      mul_a_is_inf => mul_a_is_inf_r,
      mul_b_is_inf => mul_b_is_inf_r,
      mul_a_is_zero => mul_a_is_zero_r,
      mul_b_is_zero => mul_b_is_zero_r,
      mul_rm => mul_rm,
      ...
      -- DIV Stage 2 inputs (use the registered internal signals)
      div_quotient => div_quotient,
      div_sticky => div_sticky,
      div_exp => div_exp,
      div_sign => div_sign,
      div_busy => div_busy_in,
      div_special => div_special,
      div_special_result => div_special_result,
      div_valid => div_valid,
      div_rm => div_rm,
      div_s1_valid_out => div_s1_valid_in,
