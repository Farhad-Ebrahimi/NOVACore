...
---------jump not for floats
  ...
  dmem_data_out  <= frs2_forwarded when mem_op_in = MEMOP_TYPE_STORE_FP else rs2_forwarded;
  ...
  fp_alu_x <= fp_alu_x_mux_o when (stall ='0' or internal_stall = '1') else fp_alu_x_reg;
  fp_alu_y <= fp_alu_y_mux_o when (stall ='0' or internal_stall = '1') else fp_alu_y_reg;
  ...
  begin
    if rising_edge(clk) then
      if reset = '1' or flush = '1' then
        ...
        csr_write <= CSR_WRITE_NONE;
      elsif stall = '0' or internal_stall = '1' then
