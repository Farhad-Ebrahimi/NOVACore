...
begin
	...
	stall_id <= stall_ex;    
	stall_ex <= hazard_detected or stall_mem or fp_raw_hazard_detected;
...
---------------------------------need to check the stall for csr read address------------------------
	...
	frs1_address <= id_frs1_address when stall_fpu = '0' and fp_raw_hazard_detected = '0' else
		frs1_address_p;
	frs2_address <= id_frs2_address when stall_fpu = '0' and fp_raw_hazard_detected = '0' else
		frs2_address_p;



	store_previous_frsaddr : process (clk, stall_fpu, fp_raw_hazard_detected)
		begin
		if rising_edge(clk) and stall_fpu = '0' and fp_raw_hazard_detected = '0' then
