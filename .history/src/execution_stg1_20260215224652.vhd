...
---------jump not for floats
  ...
  -- Extract FLW/FSW immediates directly from instruction
  -- FLW (opcode 00001, I-type): immediate = sign_extend(instr[31:20])
  -- FSW (opcode 01001, S-type): immediate = sign_extend({instr[31:25], instr[11:7]})
  -- For other operations: use immediate_in from decoder
  signal fpu_immediate : std_logic_vector(31 downto 0);
  
  fpu_immediate <= (31 downto 12 => instruction_in(31)) & instruction_in(31 downto 20)
                    when (instruction_in(6 downto 2) = b"00001") else  -- FLW: I-type immediate
                   (31 downto 12 => instruction_in(31)) & instruction_in(31 downto 25) & instruction_in(11 downto 7)
                    when (instruction_in(6 downto 2) = b"01001") else  -- FSW: S-type immediate
                   immediate_in;  -- For other operations, use normal immediate

  -- Calculate address  rs1 + imm for FP load/store
fp_mem_addr <= std_logic_vector(
                  unsigned(rs1_forwarded) + 
                  unsigned(fpu_immediate)  -- Use FP-specific immediate extraction
                );
