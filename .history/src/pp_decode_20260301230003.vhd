...
entity pp_decode is
	...
	port(
		...
		fp_raw_hazard_detected : in std_logic;  
...
-- Extract register addresses from the instruction word:
	-- Integer rs1 (exclude all FP instructions)
		rs1_addr <= instruction(19 downto 15)
		  when instruction(6 downto 2) /= b"10100"    ----all instructions
			...
			or(instruction(6 downto 2) = b"00001" )  -- FLW
			or(instruction(6 downto 2) = b"01001" )  -- FSW
		  else (others => '0');
		-- Integer rs2
	...
	frs1_addr <= instruction(19 downto 15)
    	when instruction(6 downto 2) = b"10100" and
    	     instruction(31 downto 25) /= b"1111000" and -- not FMV.W.X
    	     instruction(31 downto 25) /= b"1101000"     -- not FCVT.S.W
    	else (others => '0');

	frs2_addr <= instruction(24 downto 20) 
		when instruction(6 downto 2) = b"10100" 
		or instruction(6 downto 2) = b"01001" -----fsw also has rs2 as source register
		else (others => '0');  
	...
	frd_addr  <= instruction(11 downto  7) 
		...
		or 
        (instruction(6 downto 2) = b"10100" and instruction(31 downto 25) = b"1111000")  ----fmv.w.x  	
...
begin
    if rising_edge(clk) then
        if reset = '1' then
            insert_nop  <= '0';
            nop_trigger <= '0';
        elsif (stall = '0' and stall_fpu = '0' and fp_raw_hazard_detected = '0') then   ----and stall_fpu = '0'
            if is_csd_op(alu_op_reg) and rd_write_reg = '1' and  rd_addr_reg/=b"00000"  and nop_trigger = '0' then
                insert_nop  <= '1';
                nop_trigger <= '1';
				----added 2026 need to check though
			elsif is_fp_op(alu_op_reg) and frd_write_reg = '1'  and nop_trigger = '0' then
				insert_nop  <= '1';
				nop_trigger <= '1';
            else 
                insert_nop  <= '0';
