...
architecture behaviour of pp_execute is
  ...
  signal alu_op_stg1  : alu_operation;   -- registered alu_op at EX1 depth (for RAW hazard)
  signal frd_write_stg1 : std_logic;     -- registered frd write-enable at EX1 depth


  signal load_hazard_detected, csr_hazard_detected, fp_raw_hazard_detected : STD_LOGIC;
  ...
  begin
    if rising_edge(clk) then
      ...
      if stall_exe_stg1 = '0' and internal_stall = '0' then   -----internal stall added
        ...
        alu_op_stg1   <= alu_op_in;   ---- EX1 registered op for RAW hazard
        frd_write_stg1 <= frd_write_in; ---- EX1 registered frd write-enable
...
fp_alu_x_forward : process (
  ...
  begin
     ...
     --  frs1_forwarded <= rd_data_to_stg2;  -- FPU result from current Stage 3
      if frd_write_to_stg3 = '1' and frd_addr_to_stg3 = frs1_addr and (not is_fp_op(alu_op_to_stg3)) then
          frs1_forwarded <= bw_alu_result_to_stg3;  --- fcvt is done in stgae2 and its result is written here --- fcvt is done in stgae2 and its result is written here so forwarding 
      elsif frd_write_to_forwarding_stg3 = '1' and frd_addr_to_forwarding_stg3 = frs1_addr  then  ---and (not is_fp_op(alu_op_to_forwarding_stg3))
...
--    end if;                        
 ...
 begin
     -- if (stall_exe_stg1 ='0') then
       ...
       --frs2_forwarded <= rd_data_to_stg2;  -- FPU result from current Stage 3
       if frd_write_to_stg3 = '1' and frd_addr_to_stg3 = frs2_addr and (not is_fp_op(alu_op_to_stg3)) then
          frs2_forwarded <= bw_alu_result_to_stg3;  --- fcvt is done in stgae2 and its result is written here
        elsif frd_write_to_forwarding_stg3 = '1' and frd_addr_to_forwarding_stg3 = frs2_addr   then  ---and (not is_fp_op(alu_op_to_forwarding_stg3)) 
  ...
  begin
    ...
    (alu_y_src = ALU_SRC_REG and mem_frd_addr = frs2_addr )) then
      ...
      -- In detect_load_hazard, add before "end if":--
    --     elsif is_fp_op(alu_op_to_stg3) and frd_write_to_stg3 = '1'
    -- and (frd_addr_to_stg3 = frs1_addr or frd_addr_to_stg3 = frs2_addr) then
    --   load_hazard_detected <= '1';--
...
raw_hazard_detect : process (
    frs1_addr_in, frs2_addr_in,
    alu_op_stg1,   frd_write_stg1, frd_addr,
    alu_op_to_stg2, frd_write_to_stg2, frd_addr_to_stg2,
    alu_op_to_stg3, frd_write_to_stg3, frd_addr_to_stg3)
  begin
    fp_raw_hazard_detected <= '0';

    -- EX3 has the FP producer: forwarding (frd_data_stg3) takes over — release stall.
    -- This must be the highest priority so that stall from EX1/EX2 does not block forever.
    if is_fp_op(alu_op_to_stg3) and frd_write_to_stg3 = '1'
      and (frd_addr_to_stg3 = frs1_addr_in or frd_addr_to_stg3 = frs2_addr_in) then
      fp_raw_hazard_detected <= '0';   -- forwarding handles it

    -- EX2 has the FP producer: result not yet available, stall consumer at ID.
    elsif is_fp_op(alu_op_to_stg2) and frd_write_to_stg2 = '1'
      and (frd_addr_to_stg2 = frs1_addr_in or frd_addr_to_stg2 = frs2_addr_in) then
      fp_raw_hazard_detected <= '1';

    -- EX1 has the FP producer: consumer is at ID — fire stall immediately.
    -- Uses EX1-registered signals so the check fires in the same cycle fadd enters EX1.
    elsif is_fp_op(alu_op_stg1) and frd_write_stg1 = '1'
      and (frd_addr = frs1_addr_in or frd_addr = frs2_addr_in) then
      fp_raw_hazard_detected <= '1';

    end if;
  end process raw_hazard_detect;
...
--  end process detect_csd_instr_hazard;
  ...
  hazard_detected <= load_hazard_detected or csr_hazard_detected or fp_raw_hazard_detected;
