...
architecture behaviour of pp_core is
	...
	------- Stall signals -------
	signal stall_if, stall_id, stall_ex, stall_mem, stall_float : STD_LOGIC;   ----normal stall for if,id,ex and mem
...
begin
	...
	stall_float <= internal_stall or div_stall;
	...
	--------------------fpu stall----------------------
	-- FPU.cpp:62:5  ----update_stall  this is for floating point only
...
---------------------------------need to check the stall for csr read address------------------------
	...
	frs1_address <= id_frs1_address when (stall_float = '0') else
		frs1_address_p;
	frs2_address <= id_frs2_address when (stall_float = '0') else
		frs2_address_p;



	store_previous_frsaddr : process (clk, stall_float)
		begin
		if rising_edge(clk) and (stall_float = '0') then
	...
	fetch : entity work.pp_fetch
		generic map(
			...
			stall => stall_if,    
			flush => flush_if,
			--internal_stall => internal_stall,  -- for division
			--div_stall => div_stall,  -- for division
			stall_fpu => stall_float, --added 2026
			branch => branch_taken,
...
---------------------------------------------------------------------
	...
	execute : entity work.pp_execute
		port map(
			...
			stall_exe_stg1 => stall_ex, ---stall_exe_stg1_combined,
			stall_exe_stg2 => stall_mem, ---stall_exe_stg2_combined,
			flush => flush_ex,
			irq => irq,
...
------------------------need to check the stall condition ----------------
	...
	memory : entity work.pp_memory
		port map(
			...
			reset => reset,
			stall => stall_mem,
			------internal_stall => internal_stall,				-----stall_exe_stg2_combined ,
