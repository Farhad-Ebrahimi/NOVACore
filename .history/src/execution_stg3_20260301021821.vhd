...
-- FPU Stage 2 all 4 operations inputs (for final muxing by funct7)
    ...
    signal div_s1_valid_out :  std_logic;  ---- coming from stage2
    signal div_busy        : std_logic;
     
    signal instruction_in :  std_logic_vector(31 downto 0);  -- For funct7
      ...
      if reset = '1' then
       ...
       --- frd_data_out <= frd_data_in;   ---- for FPU
        ...
        dmem_read_req  <= dmem_read_req_in;
              ...
              add_exp_in   <= add_exp_in;
              add_mant_in  <= add_mant_in;
              add_rm_in    <= add_rm_in;
              add_valid_in <= add_valid_in;
              add_s1_valid_out <= add_s1_valid_out;
               
              sub_sign_in <= sub_sign_in;
              sub_exp_in <= sub_exp_in;
              sub_mant_in <= sub_mant_in;
              sub_rm_in  <= sub_rm_in;
              -- MUL Stage 2 inputs
              mul_product_in <= mul_product_in;
              mul_exp_sum_in <= mul_exp_sum_in;
