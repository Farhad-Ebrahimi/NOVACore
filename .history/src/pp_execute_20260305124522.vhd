...
--------------------------------------------------------------------------------
    ...
    fp_raw_hazard_detected  : out std_logic := '0';
...
architecture behaviour of pp_execute is
  ...
  signal fp_raw_hazard : std_logic := '0';
  ...
  signal dmem_fpdata_out_reg : STD_LOGIC_VECTOR(31 downto 0);  ---- for floats
  signal dmem_fpdata_out_2_reg : STD_LOGIC_VECTOR(31 downto 0);  ---- for floats
  signal dmem_fpdata_out_3_reg : STD_LOGIC_VECTOR(31 downto 0);  ---- for floats
...
signal frd_stg1_result        : std_logic_vector(31 downto 0); 
signal frd_data_stg2_out   : std_logic_vector(31 downto 0);    


    signal en_fadd_to_stg2 : std_logic;
    signal en_fsub_to_stg2 : std_logic;
  	signal en_fmul_to_stg2 : std_logic;
  	signal en_fdiv_to_stg2 : std_logic;


    signal en_fadd_to_stg3 : std_logic;
    signal en_fsub_to_stg3 : std_logic;
  	signal en_fmul_to_stg3 : std_logic;
  	signal en_fdiv_to_stg3 : std_logic;
...
-------------------------------------------------------------------
  ...
  signal raw_stall : std_logic := '0';
  
  attribute MARK_DEBUG : string;
  attribute MARK_DEBUG of exception_out_if: signal is "TRUE";
  -- ILA debug (trimmed to reduce BRAM)
  attribute MARK_DEBUG of frs1_forwarded       : signal is "TRUE";
  attribute MARK_DEBUG of frs2_forwarded       : signal is "TRUE";
  attribute MARK_DEBUG of frd_data_stg3        : signal is "TRUE";
...
begin
  ...
  fp_raw_hazard_detected <= fp_raw_hazard;
...
rm <= instruction_in(14 downto 12) when valid_in = '1' else "000";
  ...
  exe_stg1_instance : entity work.fp_exe_stg1
    port map(
      ...
      en_fadd_out => en_fadd_to_stg2,
      en_fsub_out => en_fsub_to_stg2,
       en_fmul_out => en_fmul_to_stg2,
       en_fdiv_out => en_fdiv_to_stg2,

       dmem_fpdata_out => dmem_fpdata_out_reg,  

      frd_data_out => frd_stg1_result,   ---- 
  ...
  exe_stg2_instance : entity work.fp_exe_stg2
    port map(
      ...
      en_fadd => en_fadd_to_stg2,
      en_fsub => en_fsub_to_stg2,
      en_fmul => en_fmul_to_stg2,
      en_fdiv => en_fdiv_to_stg2,

      en_fadd_out => en_fadd_to_stg3,
      en_fsub_out => en_fsub_to_stg3,
       en_fmul_out => en_fmul_to_stg3,
       en_fdiv_out => en_fdiv_to_stg3,
      ...
      dmem_fpdata_in => dmem_fpdata_out_reg,  
      dmem_fpdata_out => dmem_fpdata_out_2_reg, 
      ...
      frd_data_in => frd_stg1_result,   ----fpu.sv   --- just passed into stage 3 needs to be removed
      ...
      frd_data_out => frd_data_stg2_out,   
  ...
  exe_stg3_instance : entity work.fp_exe_stg3
    port map(
      ...
      dmem_fpdata_in => dmem_fpdata_out_2_reg,  
      


      en_fadd => en_fadd_to_stg3,
      en_fsub => en_fsub_to_stg3,
      en_fmul => en_fmul_to_stg3,
      en_fdiv => en_fdiv_to_stg3,
...
fp_alu_x_forward : process (
    ...
    frd_data_stg3,frd_write_to_stg2, frd_addr_to_stg2,alu_op_to_stg2,
     alu_op_to_stg3,frd_addr_to_forwarding_stg3,
    mem_frd_write, mem_frd_addr, mem_frd_value,frd_data_stg2_out,
    wb_frd_write, wb_frd_addr, wb_frd_value,frd_stg1_result,
    frs1_addr, frs1_data,rd_data_to_stg2,bw_alu_result_to_stg3
)
begin

 --if frd_write_to_stg2 = '1' and frd_addr_to_stg2 = frs1_addr then 
 --   frs1_forwarded <= frd_data_stg2_out;   

if frd_write_to_stg3 = '1' and frd_addr_to_stg3 = frs1_addr then ----and (not is_fp_op(alu_op_to_stg3)) then
         frs1_forwarded <= bw_alu_result_to_stg3;
elsif frd_write_to_forwarding_stg3 = '1' and frd_addr_to_forwarding_stg3 = frs1_addr then ---and (not is_fp_op(alu_op_to_forwarding_stg3)) then  --- forwading from stage3 to stage1
        frs1_forwarded <= frd_data_stg3;
  elsif mem_frd_write = '1'and mem_frd_addr = frs1_addr then
        frs1_forwarded <= mem_frd_value;
  elsif wb_frd_write = '1' and wb_frd_addr = frs1_addr then
        frs1_forwarded <= wb_frd_value;
    else
...
fp_alu_y_forward : process (
  ...
  frd_data_stg3,frd_write_to_stg2, frd_addr_to_stg2,alu_op_to_stg2,
   alu_op_to_stg3,frd_addr_to_forwarding_stg3,
  mem_frd_write, mem_frd_addr, mem_frd_value,frd_data_stg2_out,
  wb_frd_write, wb_frd_addr, wb_frd_value,bw_alu_result_to_stg3,
  frs2_addr, frs2_data)

begin

--if frd_write_to_stg2 = '1' and frd_addr_to_stg2 = frs2_addr then 
--     frs2_forwarded <= frd_data_stg2_out;     
--
if frd_write_to_stg3 = '1' and frd_addr_to_stg3 = frs2_addr then  ----out of ex2
         frs2_forwarded <= bw_alu_result_to_stg3;

elsif frd_write_to_forwarding_stg3 = '1' and frd_addr_to_forwarding_stg3 = frs2_addr then   ---- out of ex3

      frs2_forwarded <= frd_data_stg3;

elsif mem_frd_write = '1'and mem_frd_addr = frs2_addr then    ---- out of mem

      frs2_forwarded <= mem_frd_value;

elsif wb_frd_write = '1' and wb_frd_addr = frs2_addr then     -----out of wb
...
----- taken care of by nops and forwarding
detect_fp_raw_hazard : process (alu_op_in,
    alu_op_to_stg2,
    alu_op_to_stg3,
    frd_write_to_stg2,
    frd_write_to_stg3,
...
if ((if_fp_to_int_op(alu_op_in) and if_fp_arith_op(alu_op_to_stg2)) and (frs1_addr_in= frd_addr_to_stg2 and frd_write_to_stg2 = '1')) or 
((if_fp_to_int_op(alu_op_in) and if_fp_arith_op(alu_op_to_stg2)) and (frs2_addr_in = frd_addr_to_stg2 and frd_write_to_stg2 = '1')) then

  fp_raw_hazard <= '1';

--elsif (if_fp_to_int_op(alu_op_in) and if_fp_arith_op(alu_op_to_stg2)) and (frs2_addr_in = frd_addr_to_stg2 and frd_write_to_stg2 = '1') then
--
--  fp_raw_hazard <= '1';

elsif ((if_fp_to_int_op(alu_op_in) and if_fp_arith_op(alu_op_to_stg3)) and (frs1_addr_in = frd_addr_to_stg3 and frd_write_to_stg3 = '1')) or 
((if_fp_to_int_op(alu_op_in) and if_fp_arith_op(alu_op_to_stg3)) and (frs2_addr_in = frd_addr_to_stg3 and frd_write_to_stg3 = '1')) then

  fp_raw_hazard <= '1';

--elsif (if_fp_to_int_op(alu_op_in) and if_fp_arith_op(alu_op_to_stg3)) and (frs2_addr_in = frd_addr_to_stg3 and frd_write_to_stg3 = '1') then
--
--  fp_raw_hazard <= '1';

--elsif (if_fp_to_int_op(alu_op_in) and if_fp_arith_op(alu_op_to_forwarding_stg3)) and (frs1_addr_in = frd_addr_to_forwarding_stg3 and frd_write_to_forwarding_stg3 = '1') then
--
--  fp_raw_hazard <= '1';
--
--elsif (if_fp_to_int_op(alu_op_in) and if_fp_arith_op(alu_op_to_forwarding_stg3)) and (frs2_addr_in = frd_addr_to_forwarding_stg3 and frd_write_to_forwarding_stg3 = '1') then
--
--  fp_raw_hazard <= '1';
else 
  fp_raw_hazard <= '0';
end if;
end process detect_fp_raw_hazard;
