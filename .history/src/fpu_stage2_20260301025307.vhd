...
architecture rtl of fpu_stage2 is
    	...
    	signal add_A_sign_in       :     std_logic;
        signal add_B_sign_in       :     std_logic;
        signal add_A_exp_in        :     std_logic_vector(7 downto 0);
        signal add_B_exp_in        :     std_logic_vector(7 downto 0);
        signal add_A_mant_in       :     std_logic_vector(26 downto 0);
        signal add_B_mant_in       :     std_logic_vector(26 downto 0);
        signal add_rm_in           :     std_logic_vector(2 downto 0);
        signal add_valid_in        :     std_logic;


       signal sub_A_sign_in       :      std_logic;
       signal sub_B_sign_in       :      std_logic;
       signal sub_A_exp_in        :      std_logic_vector(7 downto 0);
       signal sub_B_exp_in        :      std_logic_vector(7 downto 0);
       signal sub_A_mant_in       :      std_logic_vector(26 downto 0);
       signal sub_B_mant_in       :      std_logic_vector(26 downto 0);
       signal sub_rm_in           :      std_logic_vector(2 downto 0);


        signal mul_A_sign_in       :     std_logic;
        signal mul_B_sign_in       :     std_logic;
        signal mul_A_exp_in        :     std_logic_vector(7 downto 0);
        signal mul_B_exp_in        :     std_logic_vector(7 downto 0);
        signal mul_A_frac_in       :     std_logic_vector(22 downto 0);  -- Raw fraction for NaN/  f detection
        signal mul_B_frac_in       :     std_logic_vector(22 downto 0);
        signal mul_A_mant_in       :     std_logic_vector(26 downto 0);
        signal mul_B_mant_in       :     std_logic_vector(26 downto 0);
        signal mul_rm_in           :     std_logic_vector(2 downto 0);


       signal div_x_val_in        :     std_logic_vector(31 downto 0);
       signal div_y_val_in        :     std_logic_vector(31 downto 0);
       ...
       signal div_rm          :    std_logic_vector(2 downto 0);
...
begin
     ...
     begin
        if rising_edge(clk) then
            ...
            add_A_sign_in <=add_A_sign;
            add_B_sign_in <=add_B_sign;
            add_A_exp_in <=add_A_exp;
            add_B_exp_in <=add_B_exp;
            add_A_mant_in <=add_A_mant;
            add_B_mant_in <=add_B_mant;
            add_rm_in <=add_rm;
            add_valid_in <=add_valid;

            sub_A_sign_in <=sub_A_sign;
            sub_B_sign_in <=sub_B_sign;
            sub_A_exp_in <=sub_A_exp;
            sub_B_exp_in <=sub_B_exp;
            sub_A_mant_in <=sub_A_mant;
            sub_B_mant_in <=sub_B_mant;
            sub_rm_in <=sub_rm;

            mul_A_sign_in <=mul_A_sign;
            mul_B_sign_in <=mul_B_sign;
            mul_A_exp_in <=mul_A_exp;
            mul_B_exp_in <=mul_B_exp;
            mul_A_frac_in <=mul_A_frac;
            mul_B_frac_in <=mul_B_frac;
            mul_A_mant_in <=mul_A_mant;
            mul_B_mant_in <=mul_B_mant;
            mul_rm_in <=mul_rm;

            div_x_val_in <=div_x_val;
            div_y_val_in <=div_y_val;
            div_exp_in <=div_exp;
    ...
    port map (
        ...
        --alu_op_in          => ALU_FADD,
        en_fadd            => en_fadd,
        A_sign_in          => add_A_sign_in,
        B_sign_in          => add_B_sign_in,
        A_exp_in           => add_A_exp_in,
        B_exp_in           => add_B_exp_in,
        A_mant_in          => add_A_mant_in,
        B_mant_in          => add_B_mant_in,
        rm_in              => add_rm_in,
    ...
    port map (
        ...
        --alu_op_in          => ALU_FSUB,
        en_fsub            => en_fsub,
        A_sign             => sub_A_sign_in,
        B_sign             => sub_B_sign_in,
        A_exp              => sub_A_exp_in,
        B_exp              => sub_B_exp_in,
        A_mant             => sub_A_mant_in,
        B_mant             => sub_B_mant_in,
        rm_in              => sub_rm_in,
        use_csd_result     => use_csd_result,
        csd_result_sign    => csd_result_sign,
        csd_result_exp     => csd_result_exp,
    ...
    port map (
        ...
        --alu_op_in          => ALU_FMUL,
        en_fmul            => en_fmul,
        A_sign             => mul_A_sign_in,
        B_sign             => mul_B_sign_in,
        A_exp              => mul_A_exp_in,
        B_exp              => mul_B_exp_in,
        A_frac             => mul_A_frac_in,
        B_frac             => mul_B_frac_in,
        A_mant             => mul_A_mant_in(23 downto 0),
        B_mant             => mul_B_mant_in(23 downto 0),
        rm_in              => mul_rm_in,
        use_csd_result     => use_csd_result,
        csd_result_sign    => csd_result_sign,
        csd_result_exp     => csd_result_exp,
    ...
    port map (
        ...
        --alu_op_in          => ALU_FDIV,
        en_fdiv             => en_fdiv,
        x_val_in            => div_x_val_in,
        y_val_in            => div_y_val_in,
        result_exp_in       => div_exp_in,
        result_sign_in      => div_sign_in,
        special_case_in     => div_special_case_in,
        special_result_in   => div_special_result_in,
        rm_in               => div_rm,  
        valid_in            => div_valid_in,   ---s1_valid_in   
