...
architecture behaviour of fp_exe_stg3 is
  ...
  signal dmem_read_req      : std_logic;
  	...
  	signal en_fadd_reg : std_logic;
  	signal en_fsub_reg : std_logic;
  	signal en_fmul_reg : std_logic;
  	signal en_fdiv_reg : std_logic;
  ...
  signal  mul_a_is_nan_reg :    std_logic;
  signal  mul_b_is_nan_reg :    std_logic;
  signal  mul_a_is_inf_reg :    std_logic;
  signal  mul_b_is_inf_reg :    std_logic;
  signal  mul_a_is_zero_reg :   std_logic;
  signal  mul_b_is_zero_reg :   std_logic;
  signal  mul_rm_reg        :    std_logic_vector(2 downto 0);
  ...
  signal alu_op_ex : alu_operation;
...
begin
  ...
  branch_out <= branch;
  alu_op_out <=  alu_op;  ---alu_op_ex when is_fp_op(alu_op_ex) else
  ...
  dmem_data_out      <= dmem_fpdata when (mem_op = MEMOP_TYPE_STORE_FP) else dmem_data;
  ...
  begin
    if rising_edge(clk) then
      if reset = '1' then
        ...
        mul_a_is_nan_reg  <= mul_a_is_nan;
        mul_b_is_nan_reg  <= mul_b_is_nan;
        mul_a_is_inf_reg  <= mul_a_is_inf;
        mul_b_is_inf_reg  <= mul_b_is_inf;
        mul_a_is_zero_reg <= mul_a_is_zero;
        mul_b_is_zero_reg <= mul_b_is_zero;
        mul_rm_reg       <= mul_rm_in;
    ...
    process (alu_op,csd_alu_result_HL,csd_alu_result_AS,bw_alu_result)
     begin
       case alu_op is
            ...
            when ALU_SLT | ALU_SLTU | ALU_AND | ALU_OR | ALU_XOR | ALU_SLL | ALU_SRL |ALU_SRA | ALU_FMVXW | ALU_FCVT_W | ALU_FCVT_WU |ALU_FLT | ALU_FLE | ALU_FEQ=>
  ...
  fpu_stg3_instance : entity work.fpu_stage3
    port map(
      ...
      en_fadd_in => en_fadd_reg,  --they are redundant not used , to be ignored
      en_fsub_in => en_fsub_reg,  --they are redundant not used , to be ignored
      en_fmul_in => en_fmul_reg,  --they are redundant not used , to be ignored
      en_fdiv_in => en_fdiv_reg,  --they are redundant not used , to be ignored
      ...
      mul_sign => mul_sign,
      mul_a_is_nan => mul_a_is_nan_reg,
      mul_b_is_nan => mul_b_is_nan_reg,
      mul_a_is_inf => mul_a_is_inf_reg,
      mul_b_is_inf => mul_b_is_inf_reg,
      mul_a_is_zero => mul_a_is_zero_reg,
      mul_b_is_zero => mul_b_is_zero_reg,
      mul_rm => mul_rm,
