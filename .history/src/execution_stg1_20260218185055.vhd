...
architecture behaviour of fp_exe_stg1 is
  ...
  -- Conversion module signals
  signal fcvt_int_result : std_logic_vector(31 downto 0);  -- FP to integer conversion result
  signal fcvt_fp_result : std_logic_vector(31 downto 0);   -- Integer to FP conversion result

  --signal div_start_in : std_logic;

begin
  
  -- Instantiate FP to integer conversion module (FCVT.W.S, FCVT.WU.S)
  fcvt_int_converter: entity work.pp_float_2_int
    port map(
      frd_data_in => frs1_forwarded,
      alu_op_in => alu_op,
      rd_data_out => fcvt_int_result
    );
