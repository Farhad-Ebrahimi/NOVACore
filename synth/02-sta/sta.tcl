...
set sdf_save_dir "./$res_dir/02-sta/sdf"
set lib_save_dir "./$res_dir/02-sta/lib"
...
read_verilog ./$res_dir/01-synth/${design_name}.v
link_design ${design_name}

read_sdc "./utils/base.sdc"

puts "\[INFO] Creating report min.rpt"
set chan [open $res_dir/02-sta/min.rpt w]
puts $chan "\n==========================================================================="
puts $chan "report_checks -path_delay min (Hold)"
puts $chan "============================================================================"
puts $chan "======================= ${current_corner} Corner ===================================\n"
close $chan
report_checks -sort_by_slack -path_delay min -fields {slew cap input nets fanout} -format full_clock_expanded -group_count 1000 -corner ${current_corner} >> $res_dir/02-sta/min.rpt


puts "\[INFO] Creating report max.rpt"
set chan [open $res_dir/02-sta/max.rpt w]
puts $chan "\n==========================================================================="
puts $chan "report_checks -path_delay max (Setup)"
puts $chan "============================================================================"
puts $chan "======================= ${current_corner} Corner ===================================\n"
close $chan
report_checks -sort_by_slack -path_delay max -fields {slew cap input nets fanout} -format full_clock_expanded -group_count 1000 -corner ${current_corner} >> $res_dir/02-sta/max.rpt


puts "\[INFO] Creating report checks.rpt"
set chan [open $res_dir/02-sta/checks.rpt w]
puts $chan "\n==========================================================================="
puts $chan "report_checks -unconstrained"
puts $chan "==========================================================================="
puts $chan "======================= ${current_corner} Corner ===================================\n"
close $chan
report_checks -unconstrained -fields {slew cap input nets fanout} -format full_clock_expanded -corner ${current_corner} >> $res_dir/02-sta/checks.rpt


set chan [open $res_dir/02-sta/checks.rpt a]
puts $chan "\n==========================================================================="
puts $chan "report_checks --slack_max -0.01"
puts $chan "============================================================================"
puts $chan "======================= ${current_corner} Corner ===================================\n"
close $chan
report_checks -slack_max -0.01 -fields {slew cap input nets fanout} -format full_clock_expanded -corner ${current_corner} >> $res_dir/02-sta/checks.rpt
...
puts $chan "======================= ${current_corner} Corner ===================================\n"
close $chan
report_check_types -max_slew -max_capacitance -max_fanout -violators -corner ${current_corner} >> $res_dir/02-sta/checks.rpt
...
puts $chan "============================================================================"
close $chan
report_parasitic_annotation -report_unannotated >> $res_dir/02-sta/checks.rpt
...
puts $chan "==========================================================================="
close $chan
check_setup -verbose -unconstrained_endpoints -multiple_clock -no_clock -no_input_delay -loops -generated_clocks >> $res_dir/02-sta/checks.rpt



puts "\[INFO] Creating report power.rpt"
set chan [open $res_dir/02-sta/power.rpt w]
puts $chan "\n==========================================================================="
puts $chan " report_power"
puts $chan "============================================================================"
puts $chan "======================= ${current_corner} Corner ===================================\n"
close $chan
report_power -corner ${current_corner} >> $res_dir/02-sta/power.rpt


set chan [open $res_dir/02-sta/skew.min.rpt w]
...
puts $chan "======================= ${current_corner} Corner ===================================\n"
close $chan
report_clock_skew -corner ${current_corner} -hold >> $res_dir/02-sta/skew.min.rpt


puts "\[INFO] Creating report skew.max.rpt"
set chan [open $res_dir/02-sta/skew.max.rpt w]
puts $chan "\n==========================================================================="
...
puts $chan "======================= ${current_corner} Corner ===================================\n"
close $chan
report_clock_skew -corner ${current_corner} -setup >> $res_dir/02-sta/skew.max.rpt


puts "\[INFO] Creating report ws.min.rpt"
set chan [open $res_dir/02-sta/ws.min.rpt w]
puts $chan "\n==========================================================================="
...
set chan [open $res_dir/02-sta/ws.max.rpt w]
puts $chan "\n==========================================================================="
...
set chan [open $res_dir/02-sta/tns.max.rpt w]
puts $chan "\n==========================================================================="
...
set chan [open $res_dir/02-sta/wiolator_list.rpt w]
puts $chan "\n==========================================================================="
...
set chan [open $res_dir/02-sta/unpropagated.rpt w]
...
set chan [open $res_dir/02-sta/clock.rpt w]
...
foreach clock [all_clocks] {
    ...
    puts $chan "============================================================================"
    close $chan
    report_clock_properties $clock >> $res_dir/02-sta/clock.rpt
    set chan [open $res_dir/02-sta/clock.rpt a]
    puts $chan "\n==========================================================================="
    puts $chan "report_clock_latency"
    puts $chan "============================================================================"
    close $chan
    report_clock_latency -clock $clock >> $res_dir/02-sta/clock.rpt
    set chan [open $res_dir/02-sta/clock.rpt a]
    puts $chan "\n==========================================================================="
    puts $chan "report_clock_min_period"
    puts $chan "============================================================================"
    close $chan
    report_clock_min_period -clocks [get_property $clock name] >> $res_dir/02-sta/clock.rpt
}
...
write_sdfs
write_libs
write_verilog "$res_dir/02-sta/${design_name}.nl.v"
write_verilog -include_pwr_gnd "$res_dir/02-sta/${design_name}.pnl.v"
